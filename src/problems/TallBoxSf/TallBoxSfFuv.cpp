/// \file TallBoxSfFuv.cpp
/// \brief The TallBoxSf galactic patch with stellar FUV and Lyman-Werner radiation.
///
...
/// the luminosities from the next step on.
///
...
template <> struct SimulationData<TallBoxSfFuv> {
...
template <> struct Particle_Traits<TallBoxSfFuv> : DefaultParticleTraits {
	static constexpr ParticleSwitch particle_switch = ParticleSwitch::StochasticStellarPop;
};

template <> struct HydroSystem_Traits<TallBoxSfFuv> {
	static constexpr bool reconstruct_eint = true; // need to reconstruct temperature
};

template <> struct quokka::EOS_Traits<TallBoxSfFuv> {
	static constexpr double gamma = 5. / 3.;
	static constexpr double mean_molecular_weight = mu;
	using EOSBackend = quokka::EOSTabulated<TallBoxSfFuv>;
};

template <> struct Physics_Traits<TallBoxSfFuv> : DefaultPhysicsTraits {
...
template <> struct RadSystem_Traits<TallBoxSfFuv> {
...
template <> void QuokkaSimulation<TallBoxSfFuv>::createInitialStochasticStellarPopParticles()
{
...
template <> void QuokkaSimulation<TallBoxSfFuv>::refineGrid(int lev, amrex::TagBoxArray &tags, amrex::Real /*time*/, int /*ngrow*/)
{
...
template <> void QuokkaSimulation<TallBoxSfFuv>::preCalculateInitialConditions()
{
...
template <> void QuokkaSimulation<TallBoxSfFuv>::setInitialConditionsOnGrid(quokka::grid const &grid_elem)
{
	...
	amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) {
		...
		const auto gamma = quokka::EOS_Traits<TallBoxSfFuv>::gamma;

		state_cc(i, j, k, HydroSystem<TallBoxSfFuv>::density_index) = rho;
		state_cc(i, j, k, HydroSystem<TallBoxSfFuv>::x1Momentum_index) = 0.0;
		state_cc(i, j, k, HydroSystem<TallBoxSfFuv>::x2Momentum_index) = 0.0;
		state_cc(i, j, k, HydroSystem<TallBoxSfFuv>::x3Momentum_index) = 0.0;
		state_cc(i, j, k, HydroSystem<TallBoxSfFuv>::internalEnergy_index) = P / (gamma - 1.);
		state_cc(i, j, k, HydroSystem<TallBoxSfFuv>::energy_index) = P / (gamma - 1.);

		for (int g = 0; g < n_groups; ++g) {
			state_cc(i, j, k, RadSystem<TallBoxSfFuv>::radEnergy_index + Physics_NumVars::numRadVarsPerGroup * g) = Erad_floor / n_groups;
			state_cc(i, j, k, RadSystem<TallBoxSfFuv>::x1RadFlux_index + Physics_NumVars::numRadVarsPerGroup * g) = 0.0;
			state_cc(i, j, k, RadSystem<TallBoxSfFuv>::x2RadFlux_index + Physics_NumVars::numRadVarsPerGroup * g) = 0.0;
			state_cc(i, j, k, RadSystem<TallBoxSfFuv>::x3RadFlux_index + Physics_NumVars::numRadVarsPerGroup * g) = 0.0;
		}

		const auto initial_scalar_density_d = initial_scalar_density;

		// Initialize passive scalar field
		if constexpr (Physics_Traits<TallBoxSfFuv>::numPassiveScalars > 0) {
			state_cc(i, j, k, HydroSystem<TallBoxSfFuv>::scalar0_index) = initial_scalar_density_d;
		}
	});
}
						     ...
						     amrex::MultiFab const &state_cc, amrex::Array<amrex::MultiFab, AMREX_SPACEDIM> const & /*state_fc*/) const
{
	...
	if (dname == "gpot") {
		...
		amrex::ParallelFor(mf, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
			Real const rho = state[bx](i, j, k, HydroSystem<TallBoxSfFuv>::density_index);
			Real const Eint = HydroSystem<TallBoxSfFuv>::ComputeInternalEnergy(state[bx], i, j, k, nullptr);
			output[bx](i, j, k, ncomp) = quokka::EOS<TallBoxSfFuv>::ComputeTgasFromEint(rho, Eint);
		});
		...
		amrex::ParallelFor(mf, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
			Real const rho = state[bx](i, j, k, HydroSystem<TallBoxSfFuv>::density_index);
			Real const Eint = HydroSystem<TallBoxSfFuv>::ComputeInternalEnergy(state[bx], i, j, k, nullptr);
			Real const Tgas = quokka::EOS<TallBoxSfFuv>::ComputeTgasFromEint(rho, Eint);
		...
		amrex::ParallelFor(mf, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
			Real const rho = state[bx](i, j, k, HydroSystem<TallBoxSfFuv>::density_index);
			Real const vz = state[bx](i, j, k, HydroSystem<TallBoxSfFuv>::x3Momentum_index) / rho;
			Real const scalar0 = state[bx](i, j, k, HydroSystem<TallBoxSfFuv>::scalar0_index);
		...
		if (dname == "true_hot_gas") {
			...
			amrex::ParallelFor(mf, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
				...
				Real const Eint = HydroSystem<TallBoxSfFuv>::ComputeInternalEnergy(state[bx], i, j, k, nullptr);
				Real const Tgas = quokka::EOS<TallBoxSfFuv>::ComputeTgasFromEint(rho, Eint);
				output[bx](i, j, k, ncomp) = (Tgas > hot_T) ? state[bx](i, j, k, HydroSystem<TallBoxSfFuv>::x3Momentum_index) : 0.0;
			});
			...
			amrex::ParallelFor(mf, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
				...
				output[bx](i, j, k, ncomp) =
				    (Tgas > warm_T && Tgas <= hot_T) ? state[bx](i, j, k, HydroSystem<TallBoxSfFuv>::x3Momentum_index) : 0.0;
			});
			...
			amrex::ParallelFor(mf, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
				...
				Real const Tgas = quokka::EOS<TallBoxSfFuv>::ComputeTgasFromEint(rho, Eint);
				output[bx](i, j, k, ncomp) =
				    (Tgas > cold_T && Tgas <= warm_T) ? state[bx](i, j, k, HydroSystem<TallBoxSfFuv>::x3Momentum_index) : 0.0;
			});
			...
			amrex::ParallelFor(mf, [=] AMREX_GPU_DEVICE(int bx, int i, int j, int k) noexcept {
				...
				Real const Eint = HydroSystem<TallBoxSfFuv>::ComputeInternalEnergy(state[bx], i, j, k, nullptr);
				Real const Tgas = quokka::EOS<TallBoxSfFuv>::ComputeTgasFromEint(rho, Eint);
				output[bx](i, j, k, ncomp) = (Tgas <= cold_T) ? state[bx](i, j, k, HydroSystem<TallBoxSfFuv>::x3Momentum_index) : 0.0;
			});
...
template <> void QuokkaSimulation<TallBoxSfFuv>::addStrangSplitSources(amrex::MultiFab &mf, int lev, amrex::Real time, amrex::Real dt_lev) // NOLINT
{
	...
	for (amrex::MFIter iter(mf); iter.isValid(); ++iter) {
		...
		amrex::ParallelFor(indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
			...
			const Real rho = state(i, j, k, HydroSystem<TallBoxSfFuv>::density_index);
			const Real x1mom = state(i, j, k, HydroSystem<TallBoxSfFuv>::x1Momentum_index);
			const Real x2mom = state(i, j, k, HydroSystem<TallBoxSfFuv>::x2Momentum_index);
			const Real x3mom = state(i, j, k, HydroSystem<TallBoxSfFuv>::x3Momentum_index);
			const Real Egas = state(i, j, k, HydroSystem<TallBoxSfFuv>::energy_index);

			static_assert(!Physics_Traits<TallBoxSfFuv>::is_mhd_enabled, "MHD is enabled; pass magnetic_energy instead of 0.0");
			const Real Eint = quokka::EOS<TallBoxSfFuv>::ComputeEintFromEgas(rho, x1mom, x2mom, x3mom, Egas, 0.0);
			...
			state(i, j, k, HydroSystem<TallBoxSfFuv>::x1Momentum_index) = x1mom_new;
			state(i, j, k, HydroSystem<TallBoxSfFuv>::x2Momentum_index) = x2mom_new;
			state(i, j, k, HydroSystem<TallBoxSfFuv>::x3Momentum_index) = x3mom_new;

			static_assert(!Physics_Traits<TallBoxSfFuv>::is_mhd_enabled, "MHD is enabled; pass magnetic_energy instead of 0.0");
			const Real Egas_new = quokka::EOS<TallBoxSfFuv>::ComputeEgasFromEint(rho, x1mom_new, x2mom_new, x3mom_new, Eint, 0.0);
			AMREX_ASSERT(!std::isnan(Egas_new));

			state(i, j, k, HydroSystem<TallBoxSfFuv>::energy_index) = Egas_new;
		});
	}
}
...
void printRadiationSummary(QuokkaSimulation<TallBoxSfFuv> &sim, std::string const &label)
{
	amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> const &dx0 = sim.geom[0].CellSizeArray();
	amrex::Real const vol = AMREX_D_TERM(dx0[0], *dx0[1], *dx0[2]);
	const amrex::Real time = sim.tNew_[0];

	// gathered on the IO processor; the first AMREX_SPACEDIM entries of each record are the particle position
	...
	constexpr int lum0 = AMREX_SPACEDIM + quokka::StochasticStellarPopParticleLumIdx;
	constexpr int birth = AMREX_SPACEDIM + quokka::StochasticStellarPopParticleBirthTimeIdx;
	constexpr int death = AMREX_SPACEDIM + quokka::StochasticStellarPopParticleDeathTimeIdx;

	std::array<amrex::Real, n_groups> L_total{};
	std::array<amrex::Real, n_groups> L_alive{}; // only stars with birth_time <= t < death_time deposit radiation
	int n_radiating = 0;
	int n_alive_radiating = 0;
	for (const auto &rdata : real_data) {
		const bool alive = time >= rdata[birth] && time < rdata[death];
		bool radiating = false;
		for (int g = 0; g < n_groups; ++g) {
			L_total[g] += rdata[lum0 + g];
			L_alive[g] += alive ? rdata[lum0 + g] : 0.0;
			radiating = radiating || (rdata[lum0 + g] > 0.0);
		}
		n_radiating += radiating ? 1 : 0;
		n_alive_radiating += (radiating && alive) ? 1 : 0;
	}
	amrex::Print() << std::format("[{}] t = {:.6e} s, particles = {}, with nonzero luminosity = {} (alive: {})\n", label, time, real_data.size(),
				      n_radiating, n_alive_radiating);

	for (int g = 0; g < n_groups; ++g) {
		const int erad_comp = RadSystem<TallBoxSfFuv>::radEnergy_index + Physics_NumVars::numRadVarsPerGroup * g;
		const amrex::Real Erad_total = sim.state_new_cc_[0].sum(erad_comp) * vol;
		const amrex::Real Erad_min = sim.state_new_cc_[0].min(erad_comp);
		const amrex::Real Erad_max = sim.state_new_cc_[0].max(erad_comp);
		amrex::Print() << std::format("[{}] group {}: L_particles = {:.6e} erg/s (alive stars: {:.6e} erg/s), E_rad total = {:.6e} erg, "
					      "E_rad min/max = {:.6e} / {:.6e} erg/cm^3\n",
					      label, g, L_total[g], L_alive[g], Erad_total, Erad_min, Erad_max);
	}
}
...
auto problem_main() -> int
{
	...
	QuokkaSimulation<TallBoxSfFuv> sim;
	...
	const amrex::Real total_gas_energy_init = sim.state_new_cc_[0].sum(HydroSystem<TallBoxSfFuv>::energy_index) * vol;
	...
	printRadiationSummary(sim, "initial");

	sim.evolve();

	printRadiationSummary(sim, "final");

	const amrex::Real total_gas_energy = sim.state_new_cc_[0].sum(HydroSystem<TallBoxSfFuv>::energy_index) * vol;
